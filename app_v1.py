...
def write_prediction(recommendation, hi_price, low_price):
    ...
    st.write("Your price is", recommendation)
    #
    st.write("### Price for car like yours is")
    st.write(low_price, '-', hi_price)
    #
...
def process_side_bar_inputs():
    ...
    # st.write(train_y_df[:1])
    # user_x_df = full_x_df[:1]
    # st.write("## Modified df")
    # st.write(full_x_df[:1])

    preprocessed_x_df = transform(full_x_df)

    # st.write("## transfomed df")
    # st.write(preprocessed_y_df[:1])
...
def sidebar_input_features():
    selling_price = st.sidebar.number_input('Введите стоимость', min_value=1)
    name = st.sidebar.selectbox("Модель", ("Ford", "Tata", "Honda"))
    ...
    year = st.sidebar.slider("Год выпуска", min_value=1980, max_value=2023, value=2018, step=1)
    # seats = st.sidebar.slider("Количетсво мест", min_value=1, max_value=20, value=4, step=1)
    seats = st.sidebar.selectbox("Количетсво мест", (7.0, 5.0, 14.0, 10.0))
    km_driven = st.sidebar.slider("Пробег", min_value=0, max_value=5000000, value=30000, step=1)
    engine = st.sidebar.slider("Объем двигателя",
                               min_value=600, max_value=3800, value=1460, step=1)
    power = st.sidebar.slider("Максимальная мощность", min_value=0, max_value=400, value=90, step=1)
    torque = st.sidebar.slider("Крутящий момент",
